...
def main():
    ...
    train_index, remaining_index, _, _ = train_test_split(
        X, y, test_size=0.20, random_state=seed
    )
    val_index, test_index, _, _ = train_test_split(
        remaining_index, y[: len(remaining_index)], test_size=0.50, random_state=seed
    )
...
#         with open(
#             os.path.join(tt_path, f"{dataset_name}_fold_{fold}_of_{folds}_train.json"),
#             "w",
#         ) as file:
#             json.dump(list(train_index.tolist()), file)
#         with open(
#             os.path.join(tt_path, f"{dataset_name}_fold_{fold}_of_{folds}_test.json"),
#             "w",
#         ) as file:
...
#         with open(
#             os.path.join(tvt_path, f"{dataset_name}_fold_{fold}_of_{folds}_train.json"),
#             "w",
#         ) as file:
#             json.dump(list(train_index), file)
#         with open(
#             os.path.join(tvt_path, f"{dataset_name}_fold_{fold}_of_{folds}_valid.json"),
#             "w",
#         ) as file:
#             json.dump(list(val_index), file)
#         with open(
#             os.path.join(tvt_path, f"{dataset_name}_fold_{fold}_of_{folds}_test.json"),
#             "w",
#         ) as file:
#             json.dump(list(test_index.tolist()), file)
