...
from torch_geometric.utils.num_nodes import maybe_num_nodes  # type: ignore
from torch_sparse import coalesce  # type: ignore
...
def to_undirected(
    ...
    edge_index, edge_attr = coalesce(
        edge_index, edge_attr, num_nodes, num_nodes, reduce
    )
    ...
    def unbatch_subgraphs(data):
        ...
        for i in range(data.num_subgraphs):
            edge_index, edge_attr = subgraph(
                torch.arange(num_nodes) + (i * num_nodes),
                data.edge_index,
                data.edge_attr,
                relabel_nodes=False,
    ...
    def to_subgraphs(self, data):
        ...
        for i in range(data.num_nodes):
            ...
            if self.add_node_idx:
                ...
                x = (
                    torch.hstack([ids, data.x])
                    if data.x is not None
                    else ids.to(torch.float)
                )
