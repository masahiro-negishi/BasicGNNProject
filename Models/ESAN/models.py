...
class DSnetwork(torch.nn.Module):
    def __init__(
        ...
        for i in range(len(channels)):
            fc_list.append(
                torch.nn.Linear(
                    in_features=channels[i - 1] if i > 0 else subgraph_gnn_out_dim,
...
class DSSnetwork(torch.nn.Module):
    ...
    def forward(self, batched_data):
        ...
        if (
            batched_data.original_edge_attr is not None
            and len(batched_data.original_edge_attr.shape) == 1
        ):
        ...
        for i in range(len(self.gnn_list)):
            ...
            tmp = torch.cat(
                [
                    torch.zeros(
                        1,
                        device=num_nodes_per_subgraph.device,
