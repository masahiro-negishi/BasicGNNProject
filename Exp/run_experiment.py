...
def get_directory(args):
    ...
    """
    return os.path.join(
        config.RESULTS_PATH, f"{args.dataset}_{os.path.split(args.grid_file)[-1]}"
    )
...
def store_error(error, errors_path, file_name, additional_info={}):
    ...
    """
    error_dict = {"error_type": str(type(error)), "error": str(error)}
...
def create_directories(directory, args):
    ...
    for split in range(args.folds):
        directory, results_path, hyperparams_path, final_eval_path, errors_path = (
            get_paths(args, split)
        )
    ...
    while param is None:
        ...
        for curr_param_to_check in stored_params:
            ...
            for key, value in param.items():
                ...
                print(
                    f"--{str(key)}",
                    curr_param_to_check["params"][f"--{str(key)}"],
                    str(value),
                )
            ...
            if same_param:
                print(
                    f"Already tried {param} and got validation score of {curr_param_to_check['val']}"
                )
                param = None
                break
...
def evaluate_params(param_dict, hyperparams_path):
    ...
    output_path = os.path.join(
        hyperparams_path,
        f"params_{len(glob.glob(os.path.join(hyperparams_path, '*')))}.json",
    )
...
def load_best_params(mode, hyperparams_path):
    ...
    for param_file in param_files:
        with open(param_file) as file:
            ...
            if (
                best_result_val is None
                or (mode == "min" and dict["val"] < best_result_val)
                or (mode == "max" and dict["val"] > best_result_val)
            ):
...
def run_final_evaluation(args, final_eval_path, best_params):
    ...
    for iteration in range(nr_tries):
        ...
        output_path = os.path.join(
            final_eval_path,
            f"eval_{len(glob.glob(os.path.join(final_eval_path, '*')))}.json",
        )
        print(output_path)
        with open(output_path, "w") as file:
...
def find_eval_params(args, grid, split):
    ...
    for c in range(nr_tries):
        ...
        try:
            ...
            nr_prev_errors = store_error(
                e, errors_path, "hyperparam_search", {"params": param_dict}
            )
