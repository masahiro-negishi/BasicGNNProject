...
def add_training_arguments(parser):
    parser.add_argument(
        "--max_time", type=float, default=12, help="Max time (in hours) for one run"
    )
    parser.add_argument(
        "--epochs",
        type=int,
    ...
    parser.add_argument(
        "--device", type=str, default="cpu", help="Which device to use(default: cpu)"
    )
    parser.add_argument(
...
def add_data_arguments(parser):
    ...
    parser.add_argument(
        "--split", type=int, default=0, help="Split for cross validation (default: 0)"
    )


def add_model_arguments(parser, model):
    parser.add_argument(
        "--drop_out", type=float, default=0.0, help="Dropout rate (default: 0.0)"
    )
    parser.add_argument(
        "--emb_dim",
        type=int,
    ...
    if model in ["GCN", "GIN", "GAT"]:
        ...
        if model == "DS":
            parser.add_argument("--invariant", type=int, default=0, help="(default: 0)")
            parser.add_argument(
                "--residual",
                type=int,
                default=0,
            ...
            parser.add_argument(
                "--channels", type=str, default="64-64", help="(default: 64-64)"
            )
    else:
...
def parse_args(passed_args=None):
    ...
    initial_args = parse_with_config_and_parsed_args(
        initial_parser, passed_args, do_parse_only_known=True
    )
    ...
    args = parse_with_config_and_parsed_args(
        parser, passed_args, do_parse_only_known=False
    )
